...
app.controller('bookedmockvisaview',function($scope,$http){
  ...
  $scope.getall=function(){
    $http({
           method : "GET",
           url : "https://arcane-ocean-14843.herokuapp.com/api/MockVisaInterviewForms",
         }).then(function(res){
           // console.log(res.data);
           $scope.data=res.data;
           $scope.originaldata=res.data
...
app.controller('sending_data',function($scope,$http){
  ...
  $http({
    method:"GET",
    url:`https://arcane-ocean-14843.herokuapp.com/api/UserSignUps/findOne?filter={"where":{"email":"${$scope.email}","password":"${$scope.password}"}}`
